...
fn verify_with_key(bytes: &[u8], signature: &[u8], public_key: &str) -> Result<(), String> {
    ...
    let signature = match Signature::decode(encoded.trim()) {
        Ok(signature) => signature,
        Err(_) => {
            // `tauri signer sign` writes the complete minisign document as a
            // single Base64 value, which is the format published by our
            // release workflow. Keep accepting plain minisign documents too.
            let decoded = base64::engine::general_purpose::STANDARD
                .decode(encoded.trim())
                .map_err(|e| format!("channel signature is invalid: {e}"))?;
            let decoded = std::str::from_utf8(&decoded)
                .map_err(|_| "decoded channel signature is not UTF-8".to_string())?;
            Signature::decode(decoded).map_err(|e| format!("channel signature is invalid: {e}"))?
        }
    };
    public_key
...
/// Resolves an authenticated remote channel. A failed request must remain an
/// error so the UI can offer a proxy retry instead of reporting an old core as
/// current from a bundled manifest.
pub async fn resolve_stable(
    ...
    .await;
    resolve_remote_bytes(&manifest?, &signature?, provider)
}
...
mod tests {
    ...
    #[test]
    fn remote_resolution_rejects_invalid_content() {
        assert!(resolve_remote_bytes(b"not json", b"not a signature", "flowseal").is_err());
    }
    ...
    #[test]
    fn verifies_tauri_base64_signature_format() {
        const KEY: &str = "RWQf6LRCGA9i53mlYecO4IzT51TGPpvWucNSCh1CBM0QTaLn73Y7GFO3";
        const SIGNATURE: &str = "untrusted comment: signature from minisign secret key\nRUQf6LRCGA9i559r3g7V1qNyJDApGip8MfqcadIgT9CuhV3EMhHoN1mGTkUidF/z7SrlQgXdy8ofjb7bNJJylDOocrCo8KLzZwo=\ntrusted comment: timestamp:1633700835\tfile:test\tprehashed\nwLMDjy9FLAuxZ3q4NlEvkgtyhrr0gtTu6KC4KBJdITbbOeAi1zBIYo0v4iTgt8jJpIidRJnp94ABQkJAgAooBQ==";
        let tauri_signature = base64::engine::general_purpose::STANDARD.encode(SIGNATURE);

        assert!(verify_with_key(b"test", tauri_signature.as_bytes(), KEY).is_ok());
        assert!(verify_with_key(b"modified", tauri_signature.as_bytes(), KEY).is_err());
    }
    #[test]
    fn checked_in_channel_signature_is_valid() {
        let manifest = String::from_utf8_lossy(include_bytes!("../../../core-channel/stable.json"))
            .replace("\r\n", "\n");
        let signature = include_bytes!("../../../core-channel/stable.json.sig");

        verify_signature(manifest.as_bytes(), signature).unwrap();
    }
    #[test]
